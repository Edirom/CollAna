...
{	if (!this.get("active"))
	...
	switch (e.key)
	{	case 'rotation':
			if(this.getMap()!= null)this.mgview_.setRotation(this.getMap().getView().getRotation());
			break;
		case 'zoomOffset':
		case 'resolution':
		{	if(this.getMap()!= null){var z = Math.max(0,this.getMap().getView().getZoom()+Number(this.get("zoomOffset")));
